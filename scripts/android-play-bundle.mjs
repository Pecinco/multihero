/**
 * Compila la web, sync Capacitor, firma release y genera app-release.aab para Play Store.
 * Requiere android/keystore.properties o play.* en local.properties o env PLAY_UPLOAD_*.
 */
import { execSync } from "node:child_process";
import { existsSync, readFileSync, statSync } from "node:fs";
import { fileURLToPath } from "node:url";
import { dirname, join } from "node:path";

const root = join(dirname(fileURLToPath(import.meta.url)), "..");
const adi = join(root, "android", "app", "src", "main", "assets", "adi-registration.properties");
const outAab = join(root, "android", "app", "build", "outputs", "bundle", "release", "app-release.aab");

if (!existsSync(adi) || !readFileSync(adi, "utf8").trim()) {
  console.error("Falta android/app/src/main/assets/adi-registration.properties (token de Play, una linea).");
  process.exit(1);
}

console.log("> npm run android:sync");
execSync("npm run android:sync", { stdio: "inherit", cwd: root, env: process.env, shell: true });

const gradle = process.platform === "win32" ? "gradlew.bat" : "./gradlew";
const android = join(root, "android");
console.log(`> ${gradle} bundleRelease`);
execSync(`${gradle} bundleRelease`, { stdio: "inherit", cwd: android, env: process.env, shell: true });

if (!existsSync(outAab)) {
  console.error("No se generó app-release.aab en la ruta esperada.");
  process.exit(1);
}
const s = statSync(outAab);
console.log(`\nOK: ${outAab}`);
console.log(`   Tamaño: ${Math.round(s.size / 1024 / 1024)} MB`);
console.log("   Sube este AAB a Play Console.");
console.log("   Antes: comprueba Firebase SHA (ver android/INSTRUCCIONES-PRODUCCION.txt).\n");
